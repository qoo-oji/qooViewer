...
struct SidePanelView: View {
    ...
    private func addToSmartLibrary(_ entry: DirectoryBrowser.Entry) {
        ...
        Task { @MainActor [smartLibraryStore, folderAccess, preferences, windowNotice, isPrivateWindow] in
            guard let result = await SmartLibraryTargetAdding.add(
...
/// ファイル名(拡張子)から行アイコンを決める。SidePanelView.iconNameとブックマークモードの
/// お気に入りツリー(SidePanelFavoriteRow)から使うため、View外の関数として切り出している。
...
private struct SidePanelPageCell: View {
    ...
    private var thumbnailPreviewContent: some View {
        ...
        .task {
            ...
            guard let loaded = await loadPageImage?(index), !Task.isCancelled else { return }
            previewImage = loaded
