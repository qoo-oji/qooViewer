...
///
/// ■ 2文以上の説明は、日本語だけ文ごとに改行を入れる(`Localizable.xcstrings` 側)
/// 英語は語のあいだで折り返せるので、放っておいても行末が語の切れ目になる。
/// 日本語にはその手がかりが無く、macOSのテキスト描画は**語の途中でも平気で折り返す**。
/// 実機では「開いている本1冊あたりの上限で / す。」「ファイルその / もの」
/// 「にだけ使ってください。す / べてを削除して」のように、
/// 文末や単語が真っ二つになっていた(スクリーンショットで確認)。
///
/// そこで、2文以上ある説明文の**日本語訳にだけ**、文の切れ目(「。」の後ろ)へ改行を入れてある。
/// 1文が1行から始まるので、上のような割れ方はしなくなる。改行位置は文の切れ目なので、
/// 環境設定ウインドウの幅を変えても、吹き出し(幅320pt固定)で見ても正しいままになる。
/// **読点(「、」)では改行しないこと。** 一度試したが、「ページ一覧で、」「オフにすると、」の
/// ような短い断片が1行を占めてしまい、かえって読みにくくなった。
///
/// 英語側には改行を入れない。語で折り返せるうえ、文ごとに改行すると右端が不揃いになるだけで
/// 得るものが無いため。
