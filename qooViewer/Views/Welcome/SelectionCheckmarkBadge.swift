...
/// (暗い丸/アクセント色の丸 ―― ウインドウが後ろなら灰色の丸)と白い縁を持っており、面を文字色で塗りつぶしても消えない。
...
struct SelectionCheckmarkBadge: View {
    ...
    let size: CGFloat
    /// 選んだ印の地の色(ウインドウが後ろなら灰色。`SelectionEmphasis`)。
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(isSelected ? SelectionEmphasis.tint(isActive: appearsActive) : Color.black.opacity(0.45))
            Circle()
