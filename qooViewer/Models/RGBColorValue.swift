...
/// 保存→復元で色がずれないようにしている。
/// `nonisolated`: 元はSwiftDataのモデル(ライブラリごとの札の地の色)のアクセサから読み書き
/// していたため。その設定はアプリ全体の環境設定へ移ってメインアクター上だけの値になったが、
/// 中身は数値3つの値型で共有状態を一切持たず、分離しない理由が無いのでそのままにしてある
/// (ArchiveReading.swift冒頭のコメントと同じ話)。
