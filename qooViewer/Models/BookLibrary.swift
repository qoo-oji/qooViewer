...
final class BookLibrary {
    ...
    ///
    /// 用途は「未分類の本をまとめておく棚」を端に固定しておくこと ―― 名前順でも更新順でも、
    /// そこだけは動かないでほしい、という要望による。指定できるのはそれぞれ1つずつ
    /// (ユーザー判断 2026-09-10。複数固定できる形は将来の判断へ回した)。
    ///
    /// ■ なぜリレーションシップではなくidか
    /// このidは**引くときに毎回`collections`の中から探し直す**(CollectionStore.
    /// pinnedCollection(_:in:))。こうしておくと、指したコレクションが削除された・別の
    /// ライブラリへ移された場合に、引けない = 「指定なし」として静かに戻るだけで済み、
    /// 参照が宙に浮いたまま残る形にならない(UUIDは作り直されないので、別のコレクションを
    /// 誤って指すこともない)。それでも、削除・移動のときには持ち主側もnilへ戻しておく
    /// (CollectionStore.clearPins(referencing:))―― 設定の面に消えたものが残らないように、
    /// また移し戻したときに固定が勝手に復活しないように。
    ///
    /// **属性の後追加なのでOptional**(SwiftDataの軽量マイグレーション。既存の行はnil =
    /// 指定なしで入る)。
    var pinnedFirstCollectionID: UUID?

    /// 常に末尾に表示するコレクションのid(nil = 指定なし)。先頭用と同じ扱い。
    /// 先頭と末尾に同じコレクションは入らない(CollectionStore.setPinnedCollection)。
