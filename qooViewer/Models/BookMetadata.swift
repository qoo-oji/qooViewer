...
///
/// 2026-09-22 から、**ファイル名を解析した本はすべて行を持つ**(利用者の指示。`isLocked` の上のコメント)。
/// ロックした行(`isLocked`)は、規則をあとから変えても内容が変わらない。ロックしていない行は、規則が変わると
/// 読み直され、利用者が直した欄(`editsData`)だけが残る。それより前は「行がある = 登録済み(ロック)」だった。
///
/// 個々の欄が空文字であること
...
final class BookMetadata {
    ...
    // すべて DB に登録し、ロックしたら変えない、削除したら消す**。それまでは「行がある = ロック(登録)」で、ロックして
    // いない本の値は保存されず(直した値は drafts.json の下書き)、画面にだけ出ていた。
    // 以前からある行はすべて利用者が登録したもの(ロック・1 冊ぶんのシート・取り込み)なので、既定値は「ロック」。

    /// ロックしているか。ロックした行の値は、規則を変えても・再生成しても・ファイルから取り込んでも変わらない。
    var isLocked: Bool = true
    /// ロックしていない行で、利用者が直した欄(qooMeta の `Confirmation` の JSON。nil なら直した欄は無い)。
    /// ロックしていない行の値は「ファイル名の解析 + この直した欄」から作り直される(規則を変えたとき・再生成)。
    var editsData: Data?
    /// 利用者が選んだルールセット(nil なら自動)。ロックした行も持ち続ける(外したときに戻るように)。
    var ruleSet: String?
    /// ファイル(EPUB/PDF/ComicInfo.xml)の持つ書誌情報を取り込んだか。取り込みは 1 冊につき 1 度だけ。
    var didImportSourceMetadata: Bool = false
