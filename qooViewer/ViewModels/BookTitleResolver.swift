...
/// DBの値、未登録ならファイル名を qooMeta で読んだタイトル(`MetadataRulesStore.reading(forBookID:rules:)`。
/// 1 冊ぶんのシートの初期値と同じ規則・同じルールセットの選び方なので、シートを開いて確かめた文字列と画面の表示が
/// 食い違うことはない。2026-09-21 に自前の正規表現の推測から qooMeta へ置き換えた)。どちらも空
...
/// (数千行の読み取りをメインアクターの外へ逃がしているメタデータの編集ウインドウと同じ理由)。一度求めたタイトルを覚えておけば、2回目からは辞書引きで済む。
///
/// ■ 捨てる契機は「読むその場での照合」だけ
/// メタデータ(`BookMetadataStore.revision`)と規則(`MetadataRulesStore.rules.contentHash`)の
...
final class BookTitleResolver {
    ...
    /// `cache`を作った時点の通し番号と規則の中身(上記)。
    private var cachedMetadataRevision: UInt64
    private var cachedRulesHash: String

    init(metadataStore: BookMetadataStore, rulesStore: MetadataRulesStore) {
    ...
    func title(forBookID bookID: String) -> String {
        ...
        let baseName = MetadataRulesStore.baseName(forBookID: bookID)
        let title = (metadataStore.metadata(forBookID: bookID)?.title
            ?? MetadataRulesStore.reading(forBookID: bookID, rules: rulesStore.rules).metadata.title)
    ...
    private func invalidateIfStale() {
        ...
        let rulesHash = rulesStore.rules.contentHash
        guard metadataRevision != cachedMetadataRevision || rulesHash != cachedRulesHash
        else { return }
