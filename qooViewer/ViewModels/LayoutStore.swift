...
final class LayoutStore: ObservableObject {
    ...
    private func existingOrNewSettings(for book: MangaBook) -> BookLayoutSettings {
        // 診断用ログ(不具合調査中の一時的な計装)。全BookLayoutSettings行のbookID一覧を出し、
        // 同じbookIDの行が複数(重複)存在していないかを確認できるようにする
        // (@Attribute(.unique)のはずだが、フェッチが直前の挿入を見落として「新規作成」の
    ...
    func setPageLayoutState(for book: MangaBook, pageKey: String, state: PageLayoutState?) {
        let bookID = book.id
        // 診断用ログ(不具合調査中の一時的な計装。原因特定後に削除予定)。この時点で
        // このbookIDに紐づくPageLayoutOverride全件(pageKey一覧)を出しておくことで、
        // 「更新」と「新規挿入」のどちらの分岐に入ったか、その直前に何件・どのpageKeyの行が
        // 存在していたかを確認できるようにする。
