...
struct QooViewerApp: App {
    ...
    var body: some Scene {
        ...
        .commands {
            ...
            // ブックマークのリネーム欄・検索欄などの実テキスト編集フィールドで実際に機能する
            // (以前はこのアプリにテキスト編集機能自体が無いという前提でこちらも空にしていたが、
            // それらのフィールドが追加されたことで前提が崩れ、Cmd+Aで全選択できない等の不具合の
            ...
            // 統合した(要望)。CommandGroup(after: .pasteboard)は、標準のカット/コピー/
            // ペースト/すべてを選択グループの直後に項目を挿入するため、先頭にDividerを置いて
            // 標準コマンドとこの後に続く独自コマンドを区切る。
        ...
        Window(String(localized: "Edit Metadata", language: locale), id: "editMetadata") {
            MetadataEditorWindow()
                ...
                .environmentObject(folderAccess)
                .environment(metadataRulesStore)
                // 並べる本と値はメタデータ生成から受け取る(2026-09-22。MetadataGenerator)。
                .environment(\.metadataGenerator, stores.metadataGenerator)
