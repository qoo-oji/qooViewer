...
nonisolated enum StoreSchemaGuard {
    ...
    /// - 2: `BookMetadata` に qooMeta の欄(2 人目以降の著者・ジャンル・イベント・原作・情報・並べ替え用の巻数)と欄の版を
    ///   足した時点(2026-09-21、ファイル名の解析を qooMeta へ置き換えたとき)。同じリリース前のブランチで
    ///   `BookReadingState.isAtLastPage` も足した(2026-09-22、スマートライブラリの「読み終えた」)。同じく
    ///   `BookMetadata` のロック・直した欄・ルールセット・取り込み済みの印も(2026-09-22、解析した本をすべて登録する)。**このブランチを
    ///   リリースするまでは、この行の指紋を書き換えてよい**(まだ誰のストアもこの世代を記録していない)
