...
- サイドパネル・ページ一覧の右クリックメニュー(「Finderで開く」。書庫・PDFの中のページは入れ物の
  ファイルを表示し、代わりに「画像をエクスポート」で1枚だけ取り出せる)
- 外観のカスタマイズ(ビューアの背景色に加えて、ページ一覧パネル・ツールバー・プログレスバー・
  サイドパネル・その他の浮かぶ表示〈「情報を見る」パネル・操作のフィードバック・拡大率の表示〉の
  背景色と「すりガラス」の濃さを、面ごとに個別に設定可能)
...
- 同じフォルダ内の前/次の本への移動
- 複数ウインドウ・複数タブ対応(Dockアイコンの右クリックからも新規ウインドウを開ける)
