...
| 4 | 書く操作の UI | **4a 済・4b 実装中**(4a: コピー/カット/ペースト・ゴミ箱・新規フォルダ・名前の変更・取り消し・進捗の帯。4b: D&D 済、残りは計画 §4) |
...
- 出し口: リストの行・ツリーの**ふつうのフォルダの行だけ**(ボリューム・ホーム・よく使う項目の根は動かさない)・アイコン表示のセル。
  ペーストボードは実際のファイルの `NSURL`。**アプリの外へはコピーだけ**を許す。アイコン表示は SwiftUI の `.onDrag` では 1 件しか運べないので、
...
| `FileBrowserOperationsTests` | ドロップの移動とコピーの混在が 1 回で戻る、⌥ で同じフォルダへの複製、コピー/カット/⌥⌘V の移動とコピーの判定、同じフォルダの複製、衝突(スキップ・両方残す)、ゴミ箱と完全削除の確認、新規フォルダの名前と編集の依頼、名前の変更と取り消し、直列、ツリーへの通知、残り時間、名前の選択範囲 |
...
- アイコン表示の名前の変更・「置き換える」は段階 4 の残り。スプリングローデッドフォルダ(ドラッグで静止して開く)は無い(ツリーの行は `NSOutlineView` の標準で開く)。「このアプリケーションで開く」は段階 8。
