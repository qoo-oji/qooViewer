...
- 「すぐに削除…」の実機確認(2026-09-23、Debug・使い捨てボリューム): ⌥ で 3 つとも入れ替わる。確認の文面・既定のボタン
  ...
  **見つけて未修正のもの**(→「既知の制限」):
  - 確認が **Esc で閉じない**(Finder の同じ確認は Esc で閉じる。実測)。「キャンセル」を既定にするためにそのボタンの
    `keyEquivalent` を `"\r"` にしたので、NSAlert が「Cancel」の題のボタンに付ける Esc が消えた(`confirmImmediateDeletion`)。
  - 確認を出している間もメニューバーの「すぐに削除…」は押せ、2 回目の操作が列に並ぶ。1 回目で消した後に、もう無い項目の確認が
    出て、「削除」を押すと「項目が見つかりませんでした」になる(メニューから 2 回再現。Esc が効かないので押し直す流れで起きる)。
  - 進捗の題の日本語訳だけが “” を使っている(ほかの 208 件は「」)。
...
- 「すぐに削除…」の確認が Esc で閉じない。確認を出している間にメニューバーから重ねて頼むと、消えた後の項目の確認が出てエラーになる
  (2026-09-23 の実機確認で見つけた。「⌥ で入れ替わる項目」)。
