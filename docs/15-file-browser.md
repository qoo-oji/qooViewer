...
| コレクションに登録 ▸ | 同上 | サブメニューにライブラリ(1 つなら省く)→ コレクション(本棚の並び順)。棚は中の本へ展開して足す(`booksToAdd`)。同じ本は `CollectionStore.add` が弾く |
...
- **zip の日時は現地時刻**(`ZipDOSTime`、2026-09-14 の実機検証で発見): MS-DOS 形式の日時はタイムゾーンを持たない現地時刻で、Finder も Info-ZIP も
  ...
  9 時間前に見えた。読むとき(展開・「情報を見る」)も書くとき(圧縮)もずらす。**CbzExporter / EpubExporter の書き出しは直していない**(書き出す時刻を渡していない)。
