...
## 保存データの JSON に入るもの(2026-09-23)

「よく使う項目」と**自動リネームの規則**は保存データの書き出しに入る(利用者の運用: JSON とコレクション表紙の組を
バックアップとして持つ → [08](08-export-and-import.md#2026-09-23-に足した-4-カテゴリ))。どちらもパスだけを書き、
取り込み側は実在するフォルダにだけ登録する(読む権限は `FolderAccessStore` が別に持つ)。

規則の対象からは `bookmark`・`volumeUUID`・**`confirmedSignature`** を落とす。**確認の印を別の端末から持ち込んでは
いけない** ―― 持ち込むと、取り込んだ直後から中身を見ずに名前を変え始める(§8 の 2)。印が無ければ利用者が確認し直すまで
何もしないので、取り込みウインドウはそのことを文章で出す。表示の状態(表示形式・アイコンの大きさ・左の幅・最後に見ていた
フォルダ)は「そのときの状態」なので入れない。
...
- **並べ替えは `FileIO` の上で**、読み直した結果が前と同じなら一覧を差し替えない(`FileBrowserState.apply(_:sortedWith:)`、2026-09-25 の監査。
  以前はメインで並べ替え、FSEvents・アクティブ化のたびに中身が同じでも `entriesRevision` を進めて表を作り直した ―― 数千項目のフォルダで
  書き込みが続くと、そのたびに `reloadData`)。絞り込みの照合に使う名前の畳み(`LibrarySearchQuery` の正規化)は項目ごとに 1 度だけ
...
その隣に「スマートライブラリの対象に追加」(`addToSmartLibrary`、2026-09-23、ユーザー要望)。フォルダだけで、まだ対象フォルダでない
ものがあるときだけ押せる(`SmartLibraryStore.containsFolder`。シークレットウインドウでは淡色、読み取り専用モードでは押せる)。
環境設定「スマートライブラリを有効にする」が OFF なら項目ごと出さず(`groups(for:includesSmartLibrary:)`)、入り口でも断る。
**1 冊の本になるフォルダ(画像フォルダ・章のフォルダ)は足さない** ―― 一覧はフォルダの中を読まないので、押したときに `FileIO` の上で
`ShelfFolderResolver.isSingleBookFolder` に訊き、本だったものは除いて、全部が本なら「対象フォルダにはできません」と伝える。
足したらペインの下の知らせで伝える。読む権限はスマートライブラリの「フォルダを追加…」と同じく `FolderAccessStore.add` に一本化する。
...
- 「すぐに削除…」の実機確認(2026-09-23、Debug・使い捨てボリューム): ⌥ で 3 つとも入れ替わる。確認の文面・既定のボタン
  (キャンセル)、ファイルメニューの項目(⌥⌘⌫)、リスト・アイコン表示の ⌥⌘⌫、ゴミ箱を経ずに消えること(ボリュームに `.Trashes` が
  できない)、進捗の題「「名前」を削除しています…」、読み取り専用で右クリック・メニューバーとも淡色・キーも効かないこと。
  **見つけて直したもの**(同じ日に修正し、実機で確かめ直した):
  - 確認が **Esc で閉じなかった**(Finder の同じ確認は Esc で閉じる。実測)。「キャンセル」を既定にするためにそのボタンの
    `keyEquivalent` を `"\r"` にしたので、NSAlert が「Cancel」の題のボタンに付ける Esc が消えていた。見えない大きさ 0 の
    ボタンに Esc を持たせ、既定のボタンを押させる(`NSAlert.makeEscapePress`。ロックされた項目の確認の「中止」も同じ)。
    付属ビュー(`accessoryView`)に置くと本文とボタンの間に約 28pt の余白が空いたので、組み上げた後のウインドウに足す。
    AX に名前の無いボタンとして出ないよう、ボタンとセルの両方の `accessibilityElement` を切る。
  - 確認を出している間もメニューバーの「すぐに削除…」は押せ、2 回目の操作が列に並ぶ。1 回目で消した後に、もう無い項目の確認が
    出て、「削除」を押すと「項目が見つかりませんでした」になっていた(メニューから 2 回再現)。列の順番が来た時点で在る項目だけを
    相手にし、1 つも無ければ黙って終える(`remove`。「ゴミ箱に入れる」も同じ道)。
  - 進捗の題の日本語訳だけが “” を使っていた(ほかの 208 件は「」)。「」に揃えた。
- **ボリュームそのもの・ボリュームがマウントされているフォルダは消さない**(2026-09-23 の 3 回目の監査の高 1)。「コンピュータ」の行でなく
  `/Volumes` をフォルダとして開いたときのボリュームの行は `isVolume` が false で、「すぐに削除…」がそのまま `removeItem` へ渡していた。
  `removeItem` はマウントの境界を越えて降りるので、**ボリュームの中身を全部消してから** rmdir だけが EBUSY で失敗し、「削除できませんでした」と
  報告する(使い捨てのディスクイメージで実測)。ゴミ箱の無い共有のマウントポイントを「ゴミ箱に入れる」も同じ道だった(以前から)。
  3 か所で断る: 淡色(`FileBrowserActions.canChange` ―― マウント表とパスの文字列だけ。カット・名前の変更も淡色になる)、入口
  (`remove` が確認の前に「ボリューム…なので削除できません」)、消すエンジン(`FileOperationService.deletePermanently` ―― 親だけ
  シンボリックリンクを解いてマウント表と比べる。`containsMountPoint`)。移動は以前から事前検査で断っている。
- **確認の後にも「ビューアで開いている本」を確かめ直す**(2026-09-23 の 3 回目の監査の中 7)。確認のシートはそのウインドウだけを塞ぐので、
  出している間に別のウインドウでその本を開けた。以前は列の順番が来たときにしか見ておらず、「すぐに削除…」では開いている本を完全に
  消していた。`asking(openBookCheck:)` が答えの後に見て断る(ゴミ箱・移動のロックと取り消せない移動の確認・名前の変更・一括リネームも同じ)。
...
- **「情報を見る」(2026-09-18、ユーザー要望)** は「Finder で開く」の下。ファイル・フォルダ・ツリーの行で、1 件以上選んでいれば押せる
  ...
  サービス **`Finder/Show Info`** に URL を載せたペーストボードを渡す(`FileBrowserActions.showInfo` → `FinderReveal.showInfo`、`NSPerformService`。スマートライブラリの右クリックも同じ)。Apple Events では
...
- **頼み直すのは出どころが変わったセルだけ**(2026-09-25 の監査)。提供役の `revision` は「どれかの表紙が変わった」という全体の合図なので、
  それだけで見えている全セル(全ウインドウ)が頼み直し・CGImage を作り直し・描き直していた(表紙の抽出中は 1 冊ごと)。今はセルが
  `sourceKey(for:kind:)`(どこから持ってくるか ―― 表紙の JPEG・指定した画像・ページ・ディスクキャッシュの鍵 ―― と、メモリを捨てた世代)を
  鍵に持ち、`revision` が進んだときは見えているセルの鍵だけ比べる(`FileBrowserIconView.refreshVisibleThumbnails`)。スマートライブラリの
  表紙のセルも同じ鍵を使う。ファイルブラウザとスマートライブラリの両方を OFF にすると、提供役のメモリ(最大 96MB)を手放す。
