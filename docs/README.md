...
| 14 | [14-library-collections.md](14-library-collections.md) | ホームの本棚(ライブラリ/コレクション)。モデル・ストア・カバー画像・ドロップの振り分け・自動登録フォルダ・編集モードの規則 |
| 15 | [15-file-browser.md](15-file-browser.md) | ホームのファイルブラウザ(改善要望7)。構成・操作エンジンと取り消し・一覧の読み込みと移動の規則・書く操作と読み取り専用モード・一括リネーム・圧縮展開・サムネイル・既存機能との接続・サンドボックスと TCC の約束・自動リネーム・保存するもの・既知の制限 |
