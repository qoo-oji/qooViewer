...
- **実蔵書を画面に出さない・撮らない。** 検証は使い捨てボリューム(`hdiutil`)に合成名のフォルダ・本を置いて行い、起動時のフォルダもそこへ向ける。
  ホームフォルダ・蔵書のボリューム・Debug に残った実ストアのライブラリ名・「フォルダのアクセス権」の一覧・ファイル選択パネルは、どれも実名が写る。
...
- 見るもの: ① ホームフォルダ(許可済み)を表示しただけでは確認が出ない ―― リストでもアイコン表示でも、ツリーでホームフォルダを開いても、よく使う項目にホームフォルダを
...
- ホームフォルダを読む許可は Debug の `qooViewer.grantedFolderBookmarks` 次第(下の「Debug に残っている許可を先に疑う」)。控えてから入れ替え、終わったら戻す。
...
- 使い捨てボリュームは **`-nobrowse` を付けずに**付ける(付けると「コンピュータ」とツリーに出ない ―― Finder と同じ規則)。
  ...
  `qooViewer.fileBrowser.lastFolderPath = /Volumes/<ボリューム>` にして、**ホームフォルダや蔵書のボリュームへ移動しない**
  (画面に実在のフォルダ名が写る)。
...
- ファイル選択ダイアログ(「アクセスを許可…」・よく使う項目の「＋」)とホームフォルダの初回の許可・TCC のダイアログは自動操作しない。
...
- メニューバーの項目の淡色は、AX で `enabled of menu item "…" of menu 1 of menu bar item "…"` を読むのが早い(撮らなくてよい)。**選択を変えてから
  1.5 秒ほど待って読む**(2026-09-19。0.6 秒では 1 つ前の選択の値が返ることがあった ―― メニューバーへの値は `AppState` の保留付きの値を通る)。
- 選択の強調(`SelectionEmphasis`)は、一覧をクリックして操作先を変える・`tell application "Finder" to activate` で後ろへ回す、の組み合わせで撮る。
  上の帯(y がウインドウの上端 + 約 40pt まで)にはライブラリの名前が出るので、撮る範囲と AX の出力から外す。
...
- ドラッグ&ドロップ(段階 4b): ドラッグは CGEvent(`leftMouseDown` → `leftMouseDragged` を 20ms 刻みで 30 回 → `leftMouseUp`)を
  ...
  **Finder はリスト表示にしてから掴む**(2026-09-23)。`cliclick` の `dd:`/`dm:`/`du:` で Finder の**アイコン表示**の項目を掴んでも
  ドラッグが始まらず(Finder 側で選ばれるだけ、アイコンも動かない)、受け口の不具合に見えた。同じ手順でリスト表示の行からなら届いた。
  Finder の中身だけを撮る(サイドバーには実在の名前が並ぶ)。
- **`sips -Z N` は長いほうの辺を N にする**(2026-09-23)。縦長に撮った範囲を幅のつもりで縮めると縮小率を取り違え、座標を逆算した
  クリックが隣の項目(「すぐに削除…」のつもりで「名前を変更」)に当たった。縮小率は「縮めた画像の幅 ÷ 撮った範囲の幅(pt)」で毎回出す。
...
- **Debug のコンテナに実ストアの写しがあるまま起動しない**(2026-09-14、段階 8.5)。ホームの帯にライブラリの名前が出る。
...
- そのウインドウが自分から共有の状態に触れないよう、`RuntimeEnvironment.isRunningTests` の間は止めてある
  (ホームは本棚で始める ―― 以前は Debug の設定がファイルブラウザだと実際のホームフォルダを読みに行っていた。音も鳴らさない)。
