...
- **ポップアップは macOS 27 SDK で組むこと**(2026-09-16、利用者からの報告)。macOS 27 で
  「設定のポップアップを変えても、閉じた面に現在の値が出ない(設定自体は変わっている)」という
  不具合が出た。原因はアプリではなく OS 側で、macOS 27 のリリースノートに
  ...
  この部品のラベルは、いちばん長い選択肢の幅を確保するために**全選択肢の非表示 `Text` を `ZStack` で
  重ねている**。旧経路はラベルのビューをそのまま描かず先頭の `Text` を題として抜き出すため、
  常に先頭の選択肢の名前が出ていたと考えられる(SwiftUI がメニューの `Text` を題・副題へ写す既知の
  平坦化規則と同じ形。ただしこの症状自体は公開された報告が無く、Apple の既知問題にも載っていない)。
