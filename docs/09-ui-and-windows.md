...
- **終了の確認は `RunningWorkRegistry` で決まる**(2026-09-26)。`AppDelegate.applicationShouldTerminate` は、利用者が始めて終わりを
  待っている作業(ファイルブラウザの操作 ―― 並んでいるものも・本の書き出し(一覧からも開いている本も)・コレクション表紙の書き出し・
  保存データの書き出しと読み込み)が 1 つでも数えられていれば、アプリモーダルの確認を出す(既定の Return と Esc は「キャンセル」、
  「終了」は赤)。以前は ⌘Q で確認なしに切れた。自動リネーム・表紙の抽出のような裏の仕事は数えない(次の起動でやり直せる)。
  **利用者が始める長い作業を足すときは `begin()` / `end(_:)` で数える。** テストは共有の数え先に触れない(`forCurrentProcess` は
  テストで nil。`FileBrowserOperations.runningWork` は差し替えられる)。
- **Dock のアイコンのクリック**(`applicationShouldHandleReopen`)は、見えているウインドウがあれば何もしない(前面に来るだけ)、
  最小化したウインドウだけなら 1 枚を自分で戻す、何も無ければ AppKit/SwiftUI に新しいウインドウを開かせる(2026-09-26。以前は
  最小化したものがあると false を返すだけで、何も戻らなかった ―― 新しいウインドウを重ねないための判定が既定の復帰まで止めていた)。
- **「開く…」⌘O は常に押せる**(2026-09-26)。本のウインドウが手前に無ければ(ウインドウが無い・環境設定などが手前)「新規ウインドウで
  開く…」と同じく新しいウインドウで開く。
- **取り返しのつかない選択肢を既定のボタンにしない。** 起動時に保存データを開けなかったときの警告は「削除せずに終了」が既定で、
  「削除して続行」は赤(2026-09-26。以前は Return 1 回で全データが消えた)。SwiftUI の `.alert` に `role: .cancel` のボタンが無いと、
  SwiftUI が何もしない「キャンセル」を自分で足す(AppKit 単体で実測)。Binding の set を捨てている警告では、それを押しても何も決まらず
  閉じられないので、何も失わない側に `role: .cancel` を付けて Esc に当てる(本の中身が変わったときのレイアウトの確認)。
