...
段階は §11 の決定を反映して 0 → 9 の順(Q12 の読み取り専用モードは段階 8.5)。段階 0・1・2・3 は済み(段階 3 の結果と引き継ぎは §3.8・§3.9)。段階 4 は 4a まで実装(§4 冒頭)。各段階は単独でビルド・テストが通り、
...
- 残り(4b 以降): D&D(アプリ内・外へ・外から + 環境設定の行)、アイコン表示のインラインリネーム、type-select(アイコン)、
