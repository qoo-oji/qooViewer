...
**ブランチの状態**: `feature/file-browser` にコミット・プッシュ済みで作業ツリーはきれい(2026-09-14、ユーザー指示。§8.1 の `0c6bbac` の次のコミット)。
...
**`d0ff788` での全体の確認(2026-09-14)**: §8.5.1 の後の 3 コミットは全テストを手元で流しておらず、「CI の Debug のジョブで流れる」と書いたが
...
**1〜3 の修正(2026-09-14、ユーザー指示「致命的な不具合をまず修正」)**: コミット・プッシュ済み(`f999e2d`)。
...
- ブランチの状態: 1〜3 が `f999e2d`、4・5(と 9)が `5968b08`、6 以降が次のコミット(「ドキュメントを更新」の指示とともにコミット・プッシュ)。
...
- **1 のハングの再現確認(2026-09-15 08:10、コミットの後、再起動直後の Mac で)**: **再現した**。32MB の exFAT(FSKit)の使い捨てイメージに
  `.stage1/a.jpg`(拡張属性付き → `._a.jpg`)を作り、`._a.jpg` を `._ghost.jpg` へ写してから `a.jpg` を消し(対の `._a.jpg` はカーネルが消す)、
  **中身が孤立した `._ghost.jpg` だけのフォルダへ `rmdir`** ―― 2 分たっても `UN` 状態のまま戻らず、fseventsd・Defender のデーモン・SystemUIServer も U 状態になった
  (copyfile も qooViewer も介さない素の操作だけで起きる)。Defender の監視を外した条件は試していない(外せない)。Web 検索では同じハングの報告は見つからず、
  近いのは FSKit の exFAT で `rmSync` の再帰削除が `._` の兄弟を巻き込んで途中で止まる件(oven-sh/bun #42062)。**修正後の `removeTransferredSource` はこの形の
  rmdir を呼ばない**(`readdir` で `._` も先に消し、空でなければ rmdir しない)。ほかの `rmdir(2)` の呼び出し(置き換えの退避の入れ物・展開の一時フォルダ・
  復旧の記録)は、中の項目を rename で出した後の入れ物で、対の `._` は rename で一緒に出るので孤立した `._` だけが残る形は作らない(コードで確認)。
  このためもう一度 Mac を再起動した(その後 U 状態のプロセス・イメージが残っていないことを確認)。結果の記録は、ユーザー指示「ドキュメントを更新してコミット・プッシュ」で
  CHANGELOG・docs/13・docs/15 と一緒にコミット・プッシュ(この節と同じコミット)。
- **実機検証(2026-09-15、ユーザー指示「実機検証して」→ 途中で「修正してから実機検証をやりなおして」)**: Debug を起動し、使い捨ての APFS スパースイメージ 2 本
  (QVTA・QVTB、合成名)だけを操作した。`defaults` は控えから戻して差分ゼロ、イメージは外して削除、撮った画像も削除。
  - **1 回目で見つけて直したもの**: 4000 件(64KB)の別ボリュームへの移動は 24 秒、その取り消しは **89 秒**で、帯のバーは 1 件ごとに 0 から満タンを繰り返した
    (ユーザー指摘「4000 の Undo が個別にインジケータ表示されるの？」)。原因は `TransferUndo.undo` が受領書 1 件ごとに `fileOps.move([1 件])` を呼び、進捗の口を
    そのまま渡していたこと。**元のフォルダごとにまとめて 1 回で運ぶ**形にした(失敗した項目を外して残りを続ける、投げたら先頭だけ 1 件で確かめる、
    組が複数なら件数だけを全体へ足す)。テスト `FileCommandsTests.moveUndoPutsBackPerFolderInOneBatch`。スキームを通した全テスト 1339 件が通った。
  - **やり直し後の結果**: 取り消しは約 25 秒(移動と同じ速さ)。帯のバーは全体の容量で進む(ただし容量の数えは件数より遅れて見える ―― 件数 20% でバー 7%。未調査)。
    中止ボタンで 2653 件のところで止めると報告は出ず、⌘Z で残り 1347 件が戻り 4000 件が揃い、「やり直す」が有効(1 回目の実機で淡色に見えた件は再現しない)。
    ⇧⌘Z のやり直しも 22 秒で全件。
  - 名前の変更中の ⌘[: リスト・アイコン表示とも、打った名前で変わってから前のフォルダが表示された(画面は移動先の一覧)。
  - 長い名前 240 件のアイコン表示: 名前は中ほどの「…」で 2 行に詰まる。大きさのスライダーを約 75 段ドラッグしている間の `sample` で、メインの `twoLineName` は
    6 秒中 643 ms(1 段あたり約 9 ms)、AX の応答は 0.14 秒で変わらず。ピンチは合成できないので見ていない。
  - `heap`: 新規ノーマルウインドウで 4000 件の移動を始め、後ろに新規フォルダを並べて ⌘W ―― 移動も新規フォルダも閉じた後に最後まで行われた。その後ウインドウの開閉を
    繰り返しても `FileBrowserState` / `FileBrowserOperations` は 2〜3、`AppState` は 2 から増えない(余分な 1 つはメニューバーの項目の閉包・SwiftUI の遅れた解放が
    直前のウインドウのものを持つだけ ―― `leaks --traceTree` で `MenuItemCallback` と `LocationBox`。`Task` の経路は無い)。一覧のビューは 1 つずつ。
  - **続けて直した 2 点(ユーザー指示「先に２点を修正してください」)**:
    1. 帯のバーの遅れ ―― エンジンの報告は件数と容量が比例していた(一時テストで 1500 件のコピーを 10% 刻みに見て一致。テストは消した)。原因は画面の側で、
       `FileBrowserOperations` の進捗の口が**報告ごとに `Task { @MainActor }` を作っていた**こと(小さなファイルは項目ごとに始まり・最初のバイト・終わりを
       間引かずに報告するので、4000 件で約 1 万回。メインが列に追いつかず、Task どうしの順番も保証されない)。`ProgressRelay`(最新の 1 件だけを 50ms 空けて渡す。
       渡しに行く Task は同時に 1 つ)に替えた。テスト `FileCommandsTests.progressRelayDeliversOnlyTheLatestInOrder`。
    2. 途中で止めた取り消しの題 ―― `MoveFilesCommand.remainingToUndo`。止めたら残りの件数(1 件ならその名前)で名乗り、戻し終えたら全部の件数へ戻る(やり直しは全部を運ぶ)。
  - **再検証(同じ条件)**: 移動の帯「4000 件中 927 件目 — 60.7 MB / 262.1 MB」のときディスク上は 955 件、「2436 件目 — 159.6 MB」のとき 2446 件、バーは容量どおり
    (21%・61%)。取り消しも「920 件目」のとき 933 件、「1458 件目」のとき 1491 件。1514 件で中止すると題が「2486 項目の移動を取り消す」になり、続きの帯も
    「2486 項目の移動を取り消しています… 2486 件中 607 件目 — 39.7 MB / 162.9 MB」、戻し終えると「4000 項目の移動をやり直す」。スキームを通した全テスト 1340 件が通った。
