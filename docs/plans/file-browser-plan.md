...
段階は §11 の決定を反映して 0 → 9 の順(Q12 の読み取り専用モードは段階 8.5)。段階 0・1・2・3 は済み(段階 3 の結果と引き継ぎは §3.8・§3.9)。段階 4 は 4a と 4b の D&D まで実装(§4 冒頭。**次の人への引き継ぎは §4.9**)。各段階は単独でビルド・テストが通り、
...
- 残り(4b): アイコン表示のインラインリネーム、type-select(アイコン)、
  「置き換える」+ 退避の復旧記録、ロックされた項目の削除の確認、Finder からのペーストを sandbox で実測、
  `untitledFolderName` の 2 つ目以降を実機の Finder と突き合わせ、実機確認(各操作・すりガラス 2 条件・`heap`)、
  段階 3 から持ち越した実機確認(§3.9)。
...
1. アイコン表示のインラインリネーム(新規フォルダ直後の名前入力も。いまはリストだけ)と type-select
