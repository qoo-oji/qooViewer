...
**ブランチの状態**: `feature/file-browser` にコミット・プッシュ済み(2026-09-14、ユーザー指示)。Unrar.swift フォークも `2d2982e` を push し、
`Package.resolved` をそこへ進めた。Debug の全テスト(1217 件)、CI と同じ Release のビルド、`scripts/ci/check-all.sh` が通る。
CHANGELOG `[Unreleased]`・MANUAL・README・CLAUDE.md・docs/11・docs/15 はここまでの内容に揃っている。`MARKETING_VERSION` は触っていない。
...
**確かめていないもの(実機)**: サブメニューの見た目(リスト・アイコン・右クリックの淡色)、NSOpenPanel がシートで出ること、進捗の帯(大きな書庫の圧縮・展開と中止)、
環境設定の行の見た目、英語表示、すりガラス 2 条件(追加した UI はメニューとパネルと環境設定だけで、どれも不透明に描かれるので対象外のはず)、
`heap` での残留、ネットワーク上の共有への圧縮・展開、Finder で作った zip(`__MACOSX` 入り)と Windows で作った CP932 の zip を実物で。
...
**次に着手する候補(順番はユーザーに選んでもらう)**:
1. 段階 6 の実機の確認(上の「確かめていないもの」)
2. 段階 8(既存機能との接続)/ 段階 7(サムネイル)。段階 8.5 は書く操作が出揃ってから(Q12)。圧縮・展開も読み取り専用モードで淡色にする対象
