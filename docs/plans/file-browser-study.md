...
| Finder が ⌘C したファイルのペースト | ~~ペーストボードの `public.file-url` にも書き手の拡張が付く**はず** [推定、同じ仕組み]。**Debug ビルドで 1 回測る**~~ → **実測(2026-09-14)**: 貼れる。読んだ URL の項目自身に読み書きの許可が付く(親には付かない)。拡張を添えないバイト列だけの URL でも同じなので、付けるのはペーストボードの側(docs/15「書く操作」)。Finder 自身にファイルの「カット」は無く(⌥⌘V「ここに項目を移動」)、アプリ内の「次のペーストは移動」で表す |
