...
§0〜§9 は検討時点(2026-09-24)の記録。**決定と実装の状況は §10**(2026-09-25)。
...
- **実物の SMB での数字**(リースの有無で変わる)。手順案: 使い捨ての合成本(ノイズの JPEG 200 枚。実在の名前を使わない)を NAS へ置き、
...
- **本番のコードでのアプリ全体の計測**(ダブルクリックから最初の見開きまで)。試作(§3.4)と同じ計り方で行う予定だったが、計測の時刻に
  画面がスリープしていてウインドウが描かれず、計れなかった。
- 実物の NAS での確認(§7 の手順)。
