...
- **シートを出したままの OFF で、押すと実行された ―― 直した。** 一括リネームのシートを出したまま OFF にすると、シートはウインドウの持ち物なので残り、
  ...
  **出している間に断る状態へ切り替わっていたら「キャンセル」扱い**にした(一括リネーム・保存先の選択・すぐに消える削除・ロックされた項目・戻せない移動)。
  ...
  - **2026-09-23 に決まりを変えた**(利用者の決定): 順番を待っている操作は、順番が来た時点で断る状態なら始めない(`enqueue`)。`asking` も出す前から
    断る状態なら出さずに断る。走っている操作(キューが空で受け付けた操作を含む)は最後までやる。名前の編集中に切り替えたら編集を取りやめる。
...
## 2026-09-22: 3 つの設定(ファイルブラウザ・スマートライブラリ・ライブラリ)の組み合わせの監査
...
|---|---|---|---|---|---|
| ON | ON | ON | 本棚・ファイルブラウザ・スマート | ファイルブラウザ │ スマート │ チップ ＋ | 切り替え 2 つ・ライブラリ・自動リネーム |
| ON | ON | OFF | 本棚・ファイルブラウザ | ファイルブラウザ │ チップ ＋ | 切り替え 1 つ・ライブラリ・自動リネーム |
| ON | OFF | ON | 本棚・スマート | 本を開く・履歴 │ スマート │ チップ ＋ | 切り替え 1 つ・ライブラリ |
| ON | OFF | OFF | 本棚 | 本を開く・履歴 │ チップ ＋ | ライブラリ |
| OFF | ON | ON | ファイルブラウザ・スマート | ファイルブラウザ │ スマート | 切り替え 2 つ・自動リネーム |
| OFF | ON | OFF | ファイルブラウザ | なし | 自動リネーム |
| OFF | OFF | ON | スマート | 本を開く・履歴 │ スマート | なし(載せる項目が無い) |
| OFF | OFF | OFF | 本棚を足す前の画面(`.classic`) | なし | なし |

### 仕事ごと

- **ライブラリだけの仕事**(存在確認・表紙の抽出・自動登録フォルダ・掃除・「ホーム」メニューの名前・ファイルブラウザの表紙の照会・
  起動時の「見つからない本」)は L だけで止まり、F・S には左右されない。スマートライブラリはもうコレクションを使わない
  (対象フォルダの本だけ)ので、L が OFF でも表紙は本そのものから引ける(FileBrowserThumbnailProvider は L が OFF なら項目から)。
- **ファイルブラウザだけの仕事**(自動リネーム・動画のサムネイルの先回り・ファイル操作)は F だけで止まる。
  **サムネイルの提供役とそのディスクキャッシュは F で止めない** ―― スマートライブラリ(S)とメタデータのシートの表紙も使う。
  「ファイルブラウザで表示」はスマートライブラリの右クリックからも F に従って消える。
- **スマートライブラリだけの仕事**(集め直し・保存した一覧・qooMeta の索引・メタデータの編集ウインドウの対象フォルダ)は S だけで
  止まり、走っている集め直しも取り消して手放す(`SmartLibraryCatalog.setFeatureEnabled`)。L・F には左右されない
  (以前は帯がライブラリの一部で、L が OFF だとスマートライブラリも出なかった)。
- **止めないもの**(どの組み合わせでも): アプリ自身のファイル操作の知らせと、保存データ・よく使う項目・スマートライブラリの対象フォルダ・
  除外フォルダの付け替え、置き換えの退避の復旧、保存データの書き出し・読み込み・削除、「このアプリが知っている本」の一覧。

### 直したもの

- 「ホーム」メニュー: L が OFF・F と S が ON のとき、切り替えの項目の直下に「自動リネームの設定…」が区切り無しで続いていた。
- (同じ日の先の監査)S を OFF にしても走っている集め直しが最後まで走り、集めた一覧がメモリに残っていた。

## 2026-09-23: ほかの機能を操作する項目の監査

スマートライブラリの右クリックに「コレクションを作成」「コレクションに登録」、ファイルブラウザの右クリックに「スマートライブラリの対象に
追加」を足したので、**ある機能の画面から別の機能を操作する項目**を洗い出し、相手の機能が OFF のときに消える(入り口でも断る)かを読んだ。

| どこから | 項目 | 相手 | OFF のとき |
|---|---|---|---|
| ファイルブラウザの右クリック・「ホーム」メニュー | コレクションを作成 / コレクションに登録 | L | 項目ごと出ない・入り口で断る・分類を待つ間に OFF なら登録しない(2026-09-23 に登録の側も見るようにした) |
| ファイルブラウザの右クリック | スマートライブラリの対象に追加 | S | 項目ごと出ない・入り口で断る・本かを調べる間に OFF なら足さない |
| スマートライブラリの右クリック(グリッド・リスト) | コレクションを作成 / コレクションに登録 | L | 項目ごと出ない(`allowsCollections`)・入り口で断る・本を確かめる間に OFF なら積まない/登録しない |
| スマートライブラリ・コレクションの中・サイドパネル・ビューア・メニューバー | ファイルブラウザで表示 | F | 項目ごと出ない(`RevealInFileBrowserAction.isFeatureEnabled` / `fileBrowserFeatureEnabled`)・`FileBrowserReveal` も何もしない |
| メニューバー「Finder で表示」「ファイルブラウザで表示」 | コレクションの中で選んだ本 | L | 本棚が出ていなければ相手が無い(`HomeMenuState.itemTargets` が `isShelfShown` を見る) |
| ホームのコピー・ドラッグ | 本の実体を渡すだけ | ― | 機能に依らない(どの機能の画面から始めても、渡すのはファイル) |
| スマートライブラリの対象フォルダの欄(ドロップ) | フォルダを対象に足す | S(自分) | シークレットウインドウでは受け口を付けない(ウインドウ全体の受け口へ回る) |
| スマートライブラリの対象フォルダの行の右クリック | ファイルブラウザで表示 | F | 出ない |
| ビューアの右クリック・メニューバー「File」 | コレクションに登録(読んでいる本) | L | 出ない。シークレットウインドウでは右クリックに出ず、メニューバーは淡色。その場限りの本は淡色 |
| サイドパネルのフォルダブラウザ | コレクションに登録(本の行) / スマートライブラリの対象に追加(本でないフォルダの行) | L / S | 出ない(シークレットウインドウでも出さない) |
| メタデータの編集ウインドウの右クリック | ファイルブラウザで表示 / コレクションに登録 | F / L | 出ない(「開く」は機能に依らない) |
| メニューバー「Finder で表示」「ファイルブラウザで表示」「メタデータの編集…」 | スマートライブラリで選んだ 1 冊 | S | スマートライブラリが出ていなければ相手が無い(`HomeMenuState.singleSmartBookTarget` が `mode == .smart` を見る) |

(2026-09-23 の続き)上の一通りを足した。**購読しない**: ビューア・サイドパネル・メタデータの編集ウインドウの「コレクションに登録」は、
名前を `HomeMenuDirectoryStore`(名前・中身が変わったときだけ知らせる写し)から引き、登録は `CollectionAddingContext`(環境値。
ストアを weak で持つだけ)で行う ―― `CollectionStore` を `@EnvironmentObject` で持つと、表紙の抽出のたびにサイドパネルの行やビューアが
組み直される。結果は `AppState.viewerNotice` → ビューアのトースト。登録はブックマークを作り終えた後にもう一度ライブラリの設定を見る
(`CollectionBookAdding.add(isStillEnabled:)`)。スマートライブラリとコレクションの中の右クリックには「このアプリケーションで開く」
「本の書き出し」も足した(ファイルブラウザと同じ並び)。

メタデータの編集ウインドウ・書き出しのウインドウがコレクションを引くのは「このアプリが知っている本」の一覧で、止めないもの(上の表)。
