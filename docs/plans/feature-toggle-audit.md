...
**実機では確かめていないもの**:
1. 確認済みの対象がある状態で OFF → 対象のフォルダへ項目を置いても名前が変わらない → ON で変わること。対象を作るにはフォルダの許可(ファイル選択
   ダイアログ)が要り、ダイアログは自動操作しない決まりなので手では組めなかった。同じ筋書きは `AutoRenameServiceTests.doesNothingWhileStopped` が
   本物の FSEvents と名前の変更で確かめている。
2. 右クリックの「自動リネーム」からの入り口、シートを出したままの OFF(ウインドウごと閉じるはず)。
3. 名前の編集中・ファイル操作の確認を出したままの OFF(§4。直していない)。
