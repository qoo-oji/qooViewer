...
> **この設定は後にアプリ全体で1つのものへ移した**(ユーザー指示 2026-09-09)。ライブラリを
> 選び直すたびに一覧の地の色が入れ替わるのは外観として落ち着かない、という判断。移動先は
> 環境設定「外観」→「パネル」→「ウェルカム画面」→「ライブラリ」
> (`AppPreferences.collectionTileBackgroundColor`)。「nil = 既定」と `CustomColorPickerSheet` を
> 使うことはそのまま。`BookLibrary.coverBackgroundColorRaw` はスキーマのマイグレーションを
> 起こさないためだけの名残として残してある。カバーの形と残す位置はライブラリごとのまま。
...
## 段階 5.23(指摘 2026-09-09). 「本を追加」の一覧は、この面で入れたぶんだけ

`AddBooksPanel` は入れ先のコレクションの中身を丸ごと並べていた。既に入っている本が並んでいると、
いま落とした本がどれなのか読めない(ユーザー指摘:「すでに追加済みのファイルは表示しないでほしい。
まぎらわしい」)。この面を開いてから入った本だけを、入った順に積む。

- 覚えておくのは **id の配列**(`addedItemIDs`)。この面を出している間に別のウインドウがその本を
  外して save すると、`CollectionItem` 本体を持ったままでは次の描き直しで落ちる
  (`CollectionDetailView.missingBook` と同じ理由)。外された本は一覧から黙って落ちる。
- **見出しの冊数は棚全体のまま**(コレクション名のすぐ隣にあるので、そちらは「入れ先がいま何冊
  持っているか」。入れるたびに増える)。
- 既に入っている本を落としたときは `CollectionStore.add(_:to:)` がパス/i ノードで弾いて返さないので、
