...
| `ReadingStateReplacementTests` | 「差し替え」の誤判定で読書位置の記録(読み方向・読書位置)とブックマークが消えないこと ―― 途中でページを除外した後、フォルダに `.DS_Store` ができた後、1.71 までの記録(除外後の枚数)からの移行。ページが本当に増えたら従来どおり差し替えとして扱うこと |
