...
| `BookExportViewModelTests` | 書き出しの**前後** ―― 読み方向と見開き強制の優先順位(DB > 開いている本 > 既定)、鍵を持つ / 持たないブックマークの解決、出力先が元ファイルと同じでも書いている間ずっと元が生きていること、失敗とスキップで出力先に手を付けないこと |
...
| `LaunchCoordinatorTests` | 開いているウインドウの登録簿 ―― 同じ本を開いているウインドウを**性質(通常 / シークレット)の一致**で探すこと、複数枚の画像をまとめた本を対象外にすること |
