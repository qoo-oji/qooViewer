...
| 元の削除の名前の取り方と変化の確認 | `contentsOfDirectory`、置く前の検証だけ | `readdir`(`._*` も)、空を確かめてから rmdir、写し始めた後に ctime が変わったファイルは残す | `._` だけのフォルダへ rmdir を呼び、検証の後の保存を消しえた(2026-09-15 の 3 回目の監査) |
