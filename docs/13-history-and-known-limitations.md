...
- **EPUB**: 固定レイアウトの画像 EPUB のみ。目次は nav.xhtml だけ(`toc.ncx` へのフォールバック
  未対応)。
- **ファイルブラウザ(段階 3 時点)**: ツリーは開いた時点の子を覚えたまま(たたんで開き直すまで追加・削除が出ない)、
  アイコンは種類の汎用アイコン、パスバーはパスの綴りのまま、「種類」列は OS の言語(→ [15](15-file-browser.md#既知の制限段階-3-時点))。
...
- **改善要望7(ウェルカム画面のファイルブラウザモード・環境設定の整理)**: 2026-09-13 に検討と実装計画を立て、
  段階 0(個人情報の流出防止の仕組み)・段階 1(環境設定の整理・帯のボタン撤去)・段階 2(ファイル操作エンジン)・
  段階 3(読むだけの画面。→ [15](15-file-browser.md))が実装済み。段階 4 以降は
  [plans/file-browser-study.md](plans/file-browser-study.md)(検討・決定事項)と
  [plans/file-browser-plan.md](plans/file-browser-plan.md)(段階ごとの実装計画)を見て進める。
