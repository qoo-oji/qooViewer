...
| ウェルカム画面の帯 | 「本を開く…」「履歴から開く」の 2 ボタン | 撤去(⌘O とファイルメニュー・サイドパネルの履歴に残る) | 改善要望7。ファイルブラウザへの切り替えを置くため |
...
| ウェルカム画面のサイドパネル | 「ウェルカム画面でも表示する」で選べた(既定 ON) | 本を開いていない間は常に出さない | 改善要望7。ファイルブラウザとフォルダブラウザを同時に見せない |
...
| 表示中のフォルダの見張り | FSEvents の配下全体のイベントで読み直し、前の列挙を取り消す | フォルダ自身と直下が変わったときだけ。読み込み中に届いたら読み終えてから 1 回 | ホームを表示していると `~/Library` の書き込みで 0.3 秒ごとに読み直し、列挙の遅いフォルダで一覧が出なくなった(2026-09-14 の監査) |
...
| ファイルブラウザの「開く」の画像フォルダの判定 | `ShelfFolderResolver.role`(一覧を組み、子フォルダの中を全部読む) | 直下と子の直下の名前だけ、保護下の子は同じ場所の中からだけ | ホームで右クリックの「開く」を選ぶだけで TCC の確認が次々に出た(2026-09-14 の 2 回目の監査) |
...
| FSEvents の続きの位置 | 見張るものが無くなっても持ち越す | 忘れる | 数時間後に戻るとその間の履歴がまとめて届いた(ホームで 180 秒に 2,587 件。2026-09-14 の 2 回目の監査) |
...
- **改善要望7(ウェルカム画面のファイルブラウザモード・環境設定の整理)**: 2026-09-13 に検討と実装計画を立て、
  ...
  追い出されたファイル、ホームの配下の FSEvents など)で、一覧と引き継ぎは
