...
| ページの並び | `.numeric` 比較(従来順) | Finder と同じ照合(正準順)。切り替えの設定は 2026-09-13 に撤去 | `_Com-title-cover.JPG` のような名前で Finder と食い違い、大文字小文字で並びが丸ごと変わった |
...
| ブロッキングするファイル I/O | `Task.detached`(協調スレッドプール) | ファイルブラウザの操作は `FileIO`(投入ごとの serial queue)。既存の呼び出し元はそのまま | コア数ぶん塞ぐとアプリの async 処理が全部止まる(qooLibrary 実測)。2026-09-13、改善要望7 段階 2 |
| ゴミ箱の有無の判定 | (qooLibrary)`url(for: .trashDirectory, create: false)` の成否 | 成功 **か** マウント表でローカル | 作ったばかりのローカルのボリュームでは問い合わせが失敗するが、ゴミ箱へは送れる(2026-09-13 実測)。新品の USB メモリで「すぐに削除されます」が出るのを防ぐ |
...
| お気に入りそのもの | メニュー・ツールバー・サイドパネル・ホームに出ていた | `FavoritesFeature.isEnabled == false` で入り口を全部閉じる。コード・データ・JSON は残す | ライブラリ/コレクションに置き換えるため(改善要望5)。消すと保存済みの登録と過去の書き出しファイルを失うので、隠すだけにした |
| ホーム | 「開く…」+ 最近開いたファイル / 最近のお気に入りの2列 | ライブラリ/コレクションの本棚。履歴は「履歴から開く」ボタンへ | 改善要望5(→ [14](14-library-collections.md)) |
| ホームの帯 | 「本を開く…」「履歴から開く」の 2 ボタン | 撤去(⌘O とファイルメニュー・サイドパネルの履歴に残る) | 改善要望7。ファイルブラウザへの切り替えを置くため |
| 表示順の切り替え | 環境設定「並び順を Finder に揃える」(1.37 で追加、2026-09-06 に既定 ON) | 撤去。表示順は常に正準順。OFF だった人の表紙は起動時に一度だけ作り直す | 改善要望7(設定の整理)。OFF を選ぶ理由が残っていなかった |
| 本を開いていない画面の名前 | 「ウェルカム画面」 | 「ホーム」(ファイルブラウザのホームディレクトリは「ホームフォルダ」)。コードの `Welcome*` と保存キーはそのまま | 2026-09-15。起動時に一度出る案内ではなく、本棚とファイルブラウザで本を探しに戻る場所になったため |
| ホームのサイドパネル | 「ウェルカム画面でも表示する」で選べた(既定 ON) | 本を開いていない間は常に出さない。ただしライブラリとファイルブラウザが両方 OFF のホームでは出す(2026-09-22) | 改善要望7。ファイルブラウザとフォルダブラウザを同時に見せない。両方 OFF ならその心配が無く、v1.42 までの使い方に戻せる(ユーザー要望) |
| 本のウインドウの提示値 | `BookOpenRequest` | `WindowContentRequest`(本 / フォルダ) | 改善要望7 段階 3。フォルダを新しいタブ/ウインドウのファイルブラウザで開くため |
| ファイルブラウザの一覧 | 計画当初は SwiftUI の `Table` / `List` | リストとツリーは `NSTableView` / `NSOutlineView`、アイコンだけ SwiftUI | `Table` の退行(スクロール・選択の遅さ)。決定事項 Q3 |
| ファイルブラウザのツリーの三角 | 子フォルダの中を見ず、全部の行に三角 | `DirectoryProbe` で直下のサブフォルダの有無を 1 回だけ調べる。保護下の場所はパスの文字列で、ネットワークはマウント表で除外 | 行き止まりの三角が多かった(ユーザー要望)。調べなかった理由(TCC・往復)は除外で避けられた。2026-09-13 |
| ファイルブラウザの書く操作 | (計画)常にファイルマネージャー | 読み取り専用モードを足し、**既定 ON** | 誤操作でファイルを動かさない(決定事項 Q12)。断るのは `FileBrowserOperations` の入り口 1 か所、画面は淡色だけ。2026-09-14 |
| アプリの外へのドラッグ | コピーだけ | 移動も許す(読み取り専用の間はコピーだけ) | Finder のウインドウ同士と挙動が違うのは期待に反する(ユーザー指摘)。移動は Finder が行う。2026-09-14 |
| 表示中のフォルダの見張り | FSEvents の配下全体のイベントで読み直し、前の列挙を取り消す | フォルダ自身と直下が変わったときだけ。読み込み中に届いたら読み終えてから 1 回 | ホームフォルダを表示していると `~/Library` の書き込みで 0.3 秒ごとに読み直し、列挙の遅いフォルダで一覧が出なくなった(2026-09-14 の監査) |
| 別ボリュームへの移動で元を消せなかったとき | 写しを片付けて失敗 | 写しを残して「両方を残しました」 | `removeItem` は途中まで消して戻さないので、元と写しの両方から兄弟が消えた(6 ファイル消失を実測。2026-09-14 の監査) |
| 「置き換える」で退避をゴミ箱へ送れなかったとき | 確認なしに完全削除 | 隠し項目のまま記録と残して報告 | 利用者が選んでいない完全削除(2026-09-14 の監査) |
| 置き換えの記録の起動時の復旧 | 退避が lstat で見えなければ記録を捨てる | ボリュームが繋がっていて ENOENT のときだけ捨てる | 外付けを繋がずに起動しただけで記録が消え、隠れた元の項目が二度と知らされなかった(2026-09-14 の監査) |
| 「このアプリケーションで開く」の候補 | ファイルの URL で引く | 名前から決めた `UTType` で引く | メインアクターの上で LaunchServices が項目を調べに行き、応答しない共有で待たされえた(2026-09-14 の監査) |
| 1 回の転送の中の同じ名前 | 2 件目の衝突の相手(1 件目)も確認・「すべてに適用」の置き換えの対象 | この操作で置いた実体なら尋ねずに両方残す | ゴミ箱の無い宛先で、移動してきた唯一の実体を確認なしに消しえた(2026-09-14 の 2 回目の監査) |
| ロックされたフォルダを含む木のコピー | CLONE で EPERM のまま失敗、書きかけの片付けも素の `removeItem` | CLONE 無しでやり直し、片付けは写った保護を外してから | 消せない書きかけが残り、置き換えでは元の項目が隠しフォルダに残った(実測。2026-09-14 の 2 回目の監査) |
| 圧縮の中の列挙と書き終わり | `enumerator(atPath:)`、ZIPFoundation の閉じる結果を信じる | 自分で歩いて失敗を拾い、置く前に `fsync` と読み直しでエントリ数を確かめる | 読めないサブフォルダを黙って飛ばした zip、ディスクフルで末尾の欠けた zip が成功として出来た(実測。2026-09-14 の 2 回目の監査) |
| 展開の名前決め | 要素ごとの再帰、パス全体の長さに上限なし | ループ、パス全体を PATH_MAX まで | 3000 段の入れ子のエントリ 1 つの zip でスタックが溢れてアプリごと落ちた(実測。2026-09-14 の 2 回目の監査) |
| コピーの書き込み先 | 宛先の名前へ直に `copyfile(EXCL)`、元の検証と片付けは置いた後 | 同じフォルダの一時名へ写し、置く前に検証、`RENAME_EXCL` で置く | EXCL は同名のフォルダの中へ合流して書くので、衝突の確認の後に現れた他人のフォルダへ混ざり、片付けがそのフォルダごと消しえた(2026-09-14 の 2 回目の監査) |
| 元の変化の判定(ローカルの元) | 大きさ・実体が同じなら 64KB × 3 窓の抜き取り | 更新日時(ns)の違いも変化に数える | 窓の外だけの書き込み(領域を先に確保するダウンロード)を見逃し、移動で失いえた。日時の差し替えは SMB だけの事情(2026-09-14 の 2 回目の監査) |
| 別ボリュームへ移動したフォルダの元の削除 | `removeItem` で木ごと | 写した先にもある項目だけを下から | 写し終えてから消し終えるまでに元へ作られた項目も消えた(2026-09-14 の 2 回目の監査) |
| 元の削除の名前の取り方と変化の確認 | `contentsOfDirectory`、置く前の検証だけ | `readdir`(`._*` も)、空を確かめてから rmdir、写し始めた後に ctime が変わったファイルは残す | `._` だけのフォルダへ rmdir を呼び(exFAT(FSKit)ではその rmdir が戻らず Finder まで固まる ―― 素の操作で再現、再起動が要る)、検証の後の保存を消しえた(2026-09-15 の 3 回目の監査) |
| 取り消しの中止 | 途中で止めると履歴から外す、戻し終えた直後の中止を「戻せなかった」と数える | 戻した分を外して履歴へ戻す | 残りを ⌘Z で戻せず、置き換えた元がゴミ箱に残った(2026-09-15 の 3 回目の監査) |
| 別ボリュームへ移動した元の変化の確かめ方 | どのローカルでも「写し始めた後に ctime が変わった」 | APFS などは ctime と時刻、FAT・exFAT は大きさと更新日時を写しと比べる | FAT・exFAT の ctime は更新日時そのもの(実測)で、未来の日時のファイルで移動が 1 件目で止まり、日時を保つ上書きを見逃して消しえた(2026-09-15 の 4 回目の監査) |
| 移動の取り消しのまとめ運びが断られたとき | 先頭 1 件を試してから残り全体で呼び直す | まとまりを半分ずつに割る | 空き容量の不足などで、1 件ごとに残り全部の事前検査をやり直し項目数の 2 乗になった(2026-09-15 の 4 回目の監査) |
| 圧縮の検証の開き直し | ZIPFoundation にそのまま開かせる | 末尾の窓に EOCD があるかを先に見る | 末尾の欠けた大きな zip で全体を遡り、500MB で 20 秒・中止も効かなかった(実測。2026-09-15 の 3 回目の監査) |
| まとめた操作の途中の失敗 | 投げる(積まない) | 済んだ子があれば一部だけ済んだとして返して積む | 済んだ移動を ⌘Z で戻せず報告にも出なかった(2026-09-14 の 2 回目の監査) |
| 操作の最中に押した ⌘Z | 列の後ろで一番上を戻す | 押した時点の一番上のときだけ戻す | 走っていた操作を終わった直後に戻していた(2026-09-14 の 2 回目の監査) |
| 取り消し・やり直しの進捗 | 帯も中止も無し、やり直しは実行時の中止の旗を使い回す | 帯と中止、やり直しは新しい旗 | 別ボリュームの移動の取り消しが黙って全量を写し直し、中止した操作のやり直しは何もしなかった(2026-09-14 の 2 回目の監査) |
| ウインドウを閉じた後の操作の相手 | `presenter = nil`(報告を捨て、衝突は黙ってスキップ) | 確認は断り、衝突は止め、報告は届ける | 「元の項目を削除できなかった」などの報告が消えた(2026-09-14 の 2 回目の監査) |
| ゴミ箱の無い場所の取り消しの失敗 | 試し直せるとして履歴に残す | 残さない | 履歴の一番上に居座り、下の操作へ ⌘Z が届かなかった(2026-09-14 の 2 回目の監査) |
| ファイルブラウザのメニューの値 | `MenuCheckmarkState` へ直に詰める | `AppState` で `MenuBarMenuGate` を通す | メニューを開いている間に「移動」メニューの項目数が変わりうる(2026-09-14 の 2 回目の監査) |
| ファイルブラウザの「開く」の画像フォルダの判定 | `ShelfFolderResolver.role`(一覧を組み、子フォルダの中を全部読む) | 直下と子の直下の名前だけ、保護下の子は同じ場所の中からだけ | ホームフォルダで右クリックの「開く」を選ぶだけで TCC の確認が次々に出た(2026-09-14 の 2 回目の監査) |
| 名前の衝突を避ける番号の探し方(一括リネーム・展開) | 項目ごとに 2 から | 前の続きから | 同じ候補が並ぶと 2 乗(一括リネーム 5000 件で 4.6 秒、展開 4000 件で 2.9 秒。2026-09-14 の 2 回目の監査) |
| 一覧・ツリーの読み直しが重なったとき | 前の列挙を残したまま次を始める | 読み終えてから 1 回だけ | 応答しない共有・ダウンロードの続くフォルダで FileIO のスレッドが積もった(2026-09-14 の 2 回目の監査) |
| ネットワークの項目の絵 | 同時 4 件の枠をローカルと共有 | 別の枠(2 件) | 応答しない共有の 4 件で全ウインドウの絵が止まった(2026-09-14 の 2 回目の監査) |
| FSEvents の続きの位置 | 見張るものが無くなっても持ち越す | 忘れる | 数時間後に戻るとその間の履歴がまとめて届いた(ホームフォルダで 180 秒に 2,587 件。2026-09-14 の 2 回目の監査) |
| ファイルブラウザの書庫の絵の読み方 | 画像エントリ自身の 64MB だけ | 前にある量(256MB)・BCJ2 のブロック(64MB)にも上限、間引けない形式の画像は 3200 万画素まで | 細工された・巨大な書庫や BMP で、フォルダを表示しただけで 1 件あたり数百 MB〜2GB を確保した(実測。2026-09-14 の 2 回目の監査) |
| 展開で読み飛ばすエントリ | 限度に数えない | 宣言サイズを限度に数える | ソリッドの書庫で、捨てるエントリに隠した伸長爆弾が限度を通り抜けた(2026-09-14 の 2 回目の監査) |
| 動画の絵の先回りとディスクキャッシュ | 上限を知らずに書く | 上限の半分まで | 刈り込みと作り直しが起動のたびに続いた(2026-09-14 の 2 回目の監査) |
| ゴミ箱から戻すときの確認 | 場所に何かあれば戻す | 送った直後の実体と一致するときだけ | ゴミ箱を空にした後で同じ名前を捨てると、⌘Z が別の項目を戻した(2026-09-14 の 2 回目の監査) |
| SwiftUI の受け口の他のアプリからの移動 | 常に移動を許す | Finder からのときだけ | コピーしか許さないアプリから落とした項目を移動しえた(2026-09-14 の 2 回目の監査)。翌日アイコン表示を AppKit にしたので、効くのは一覧の外(操作列など)だけ |
| ファイルブラウザのアイコン表示の実装 | SwiftUI の `LazyVGrid`(選択・帯・矢印・名前の編集・ドラッグの開始・右クリック・キー監視を自前) | `NSCollectionView`(受け口・メニュー・名前の欄・キーの割り当てをリストと共有) | リスト表示と挙動がずれた(ドロップでドラッグ元のマスクを見られない、淡色のサブメニューに矢印が出ない、など)。2026-09-15、ユーザー判断 |
| ボリュームのルートの移動 | 断らなかった(`paste` は画面の除外も無し) | 事前検査でマウントポイントを断る | 全部写してから元のボリュームを空にした(実測。2026-09-14 の 2 回目の監査) |
| ファイルブラウザの絵のディスクキャッシュ(シークレットウインドウ) | 書いていた | 書かない(読むのは許す) | 本のページのサムネイルと揃える。絵そのものが痕跡になる(2026-09-14、ユーザー判断) |
| 「すべてのデータを削除」と「置き換える」の退避の記録 | 対象から漏れていた | 終了時に消す | 「このアプリが保存したすべて」に含める(2026-09-14、ユーザー判断) |
| 淡色のサブメニュー(SwiftUI の右クリック) | `Menu` に `.disabled` | 押せない `Button` | `.contextMenu` の中の `Menu` には `.disabled` が効かない(10 通りの書き方で実測。2026-09-14) |
...
- **ファイルブラウザ**(2026-09-14 時点): アイコンは種類の汎用アイコン、パスバーはパスの綴りのまま、「種類」列は OS の言語、
  検索は現フォルダだけ、スプリングローデッドフォルダと Dock のゴミ箱へのドラッグは無い、取り消しの受領書はパスで持つ(外で名前を変えられた
  項目は戻せない)、許可の無い場所から来た項目の移動は取り消せない。
  一覧は [15「既知の制限」](15-file-browser.md#既知の制限)。
...
- **並び順**: レイアウトのある本は、作ったときの並びに固定される(`pinPageOrderIfNeeded`)。撤去した
  「Finder に揃える」を OFF で使っていた本も、固定済みならその並びのまま。
...
- **本を表示したウインドウを閉じると、`AppState` が1つ残る**(実測 2026-09-13。小さな残留。
  ...
  持っている)で、アプリ側から切る手は見つけていない。`AppState.deinit` が閉じるはずのセキュリティ
  スコープ付きアクセスは、赤い閉じるボタン(`forceCloseWindow`)でも Cmd+W と同じく `closeBook()` を
  先に通すようにして、deinit を待たずに閉じる(同日)。残るのは `AppState` の入れ物と弱参照だけ。
  - **経緯**: 当初は閉じるたびに `AppState`・`ViewerViewModel`・`PageLoader`・SwiftUI のビューグラフ・
    ...
    増えていた。ホームだけのウインドウでは増えない。真因は**ツールバーのボタン・右クリック
...
- **改善要望7(ホームのファイルブラウザモード・環境設定の整理)**: 2026-09-13 に検討と実装計画を立て、
  2026-09-14 までに段階 0〜8.5(流出防止の仕組み・環境設定の整理・操作エンジン・画面・書く操作・一括リネーム・圧縮展開・
  サムネイル・既存機能との接続・読み取り専用モード)とブランチ全体のコード監査の修正が入り、段階 9 の文書化まで済んだ
  (→ [15](15-file-browser.md))。**残りは段階 9 の実機の検証**(`heap` でのリーク、すりガラス 2 条件の全部品、TCC の確認の出方、
  追い出されたファイル、ホームフォルダの配下の FSEvents など)で、一覧と引き継ぎは
  [plans/file-browser-plan.md](plans/file-browser-plan.md) の §9.1。検討と決定事項は [plans/file-browser-study.md](plans/file-browser-study.md)。
- **効率の監査(2026-09-25)で見送ったもの**: 背景のウインドウ・タブのページ画像のキャッシュを見えない間だけ小さくする、
  覆われたタブの見張りを止める、サイドパネルへ渡す閉包の組み替え、拡大鏡の結合画像をやめる、など。直したものと、
  見送ったものそれぞれの理由(多くは利用者の判断か実機での確認が要る)は [plans/efficiency-audit-2026-09-25.md](plans/efficiency-audit-2026-09-25.md)。
- **スマートライブラリの前回の一覧(`catalog.json`)を先に出せていない**(2026-09-26 の実機の確認で判明、未修正): 読み込みが、
  同時に始まる集め直しの `generation += 1` で毎回捨てられる。2026-09-22 から効いていないと見られ、効率の監査の「最初の集め直しで
  書き直さない」も働かない。原因と直し方の案は [plans/efficiency-audit-2026-09-25.md](plans/efficiency-audit-2026-09-25.md)「効いていないもの」。
...
**B1 で分かったこと**
- **読み方向が絡むテストは、必ず本ごとの上書きで明示する。** 既定は表示言語(テストでは
  システムの言語)から決まる(手元は右開き・CI は左開き)ので、見開き左右の期待値がそのままでは環境で食い違う。
...
- **時間で待たない。** `settle()` / `await openTask?.value` のように、合図は処理そのものから出す
  (段階 2 参照)。
- **環境で変わる既定値は明示する。** 読み方向の既定は表示言語(テストではシステムの言語)から決まる(手元は右開き、
