...
| `FileOperationService.shared` | なし(状態を持たない actor) | ファイルブラウザのコピー・移動・名前の変更・ゴミ箱・完全削除(改善要望7 段階 2。画面からは段階 4 の `FileBrowserOperations` が呼ぶ)。取り消し・やり直しの `FileCommandStack` は**ウインドウごと**(`FileBrowserState` が持つ) |
