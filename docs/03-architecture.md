...
| `FileBrowserVideoThumbnailWarmer` | なし(作った絵は `FileBrowserThumbnailDiskCache` へ) | よく使う項目の中の動画の絵を、起動している間に裏で 1 本ずつ作る(段階 7b。**テスト中は繋がない**) |
| `ReplaceBackupJournal.shared` / `ReplaceBackupRecovery` | ディスク(Application Support の JSON) | 「置き換える」の退避の記録と、起動時の 1 回の復旧(段階 4b。`FolderAccessStore` が許可を開いた後に走る。テスト中は動かさない) |
...
- **同じウインドウのほかの状態**: `WelcomeLibraryState`(本棚の表示の状態と、本棚/ファイルブラウザのモード)、
  `FileBrowserState`(ファイルブラウザの現在のフォルダ・一覧・選択・戻る/進む、書く操作の窓口
  `FileBrowserOperations` と取り消しの `FileCommandStack`。`willClose` で `releaseResources()`)、`SidePanelBrowserState`。どれも `ContentView` の `@StateObject` で、本を開いている
...
**起動時の位置・サイズは、`.defaultSize` / `.defaultPosition` にも前回終了時の値を渡します**
