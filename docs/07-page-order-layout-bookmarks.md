...
シリーズ名と巻の表記のある本だけ(表記の無い本の数は `BookMetadataValues.trimmed` が落とすので)、空にすると表記から読んだ数へ戻る。
...
- **解析した本はすべて登録**(利用者の指示 2026-09-22。それまでの「ロック = 登録」を置き換えた)。「表示されているが保存されて
  ...
  - 登録する所: この窓(開いたとき `registerAll`、以後は変わった本)、スマートライブラリ(集め直すたびに、行の無い本とロックして
    ...
    初回はどちらも数千冊を書くので、500 件ずつ区切って合間にメインを譲る(`BookMetadataStore.upsertAllInBatches`。1 回で書くと
    本番の写しの 2,207 行で 362 ms 止まった)。スマートライブラリは、記録の残るウインドウに出ている間だけ書き(`persistingCount`)、
    **この起動の間に同じ本へ同じ値を書くのは 1 度だけ**(`lastRegistered`。往復で変わる値でも書き続けない。「メタデータを削除」の
    知らせで走った集め直しが、消した行を作り直さない)。
  ...
  - **読みだけの行は起動時に間引く**(`BookMetadataStore.pruneParsedOnlyRows`、`AppStores.pruneParsedOnlyMetadata`。2026-09-22 の
    ...
    のうち、行のほかに本を覚えている理由(`KnownBooks.collect(…, includingMetadata: false)`)が無く、スマートライブラリの対象フォルダ
    (機能が OFF でも)の外の本の行を消す。この窓の一覧は `KnownBooks` で、そこに行を持つ本を入れると「行があるから一覧に出て、
    一覧に出るから行が残る」の輪になり、読書位置を間引かれた本・対象から外したフォルダの本の行が増え続けた。
  - 外で行が消えた知らせで一覧から外すのは、DB に行があると分かっている本だけ(`MetadataWorkspace.persisted`。全欄が空の本は
    行を作れないので、以前は bookID の無い知らせのたびに一覧から消えた)。
