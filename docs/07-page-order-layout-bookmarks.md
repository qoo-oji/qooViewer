...
(その場所にあって中を読めて、本ではないと確かめられたものだけ。無い・読めないものは消さない)。「確かめられた」は、直下に画像が無く
書庫・PDF・EPUB がある棚か、子フォルダを**すべて読み切って**どれの直下にも画像が無い中間のフォルダだけ(`BookExistenceProbe.isNonBookFolder`)。
2026-09-23 の 3 回目の監査(中 2)までは `isSingleBookFolder` の false をそのまま使っていて、章のフォルダが 1 つ読めないだけで章ごとに画像を
分けた本を棚と取り違え、画像を外へ出して `Icon\r` などが残ったフォルダも「空ではない」と数えていた。ルールセットは本ごとに
...
**開き方**: 編集メニューの「メタデータの編集…」は**いつもこのウインドウを開きます**(2026-09-23、利用者の指示)。
ホーム画面で本を 1 冊選んでいるとき(ファイルブラウザの選択、またはコレクションの中の 1 冊)は、その本が DB に登録してあれば
その行を選び、見える位置までスクロールします(`MetadataEditorReveal` → `MetadataWorkspace.reveal` → `MetadataBookTable`。
絞り込み・検索で隠れている行なら、それらを外してから選ぶ ―― 選んだ本が一覧に出ないまま選択だけが変わるのを避けるため)。
一覧に無い本(棚やふつうのフォルダを選んでいるときなど)なら、ただ開くだけです。1 冊ぶんのシート(`BookMetadataSheet`)は
右クリックの「メタデータの編集…」に残ります ―― 以前は編集メニューも、ホーム画面で 1 冊選んでいるとシートを出していました
(2026-09-15)。
...
変えられる。一覧では、ロックした行の文字を黄色にする(`MetadataBookTable.CellView.isLockedRow`。明るい外観では白地で読める暗めの黄色。選んだ行では強調の地に合わせた色。最初は行の地を黄色にしたが、望まれていたのは文字の色だった)。
**シリーズ名を別の名前に変えると、巻を新しいシリーズ名で読み直す**(巻数(表示)も並べ替え用の巻数も確定を外し、qooMeta の提案に
任せる ―― タイトルの新しいシリーズ名の後ろから読んだ巻、読めなければ推定。一覧・欄をまとめて変更・1 つのシリーズにする・シートの
どこからでも。2026-09-22、利用者の指示: 巻はシリーズの中の番号なので、前のシリーズの巻を持ち越さない。いったんは巻を消したが、
提案に戻すほうが望まれた)。表記だけの直し
(空白・記号・全角半角・大文字小文字の違いだけ。`MetadataWorkspace.sameSeriesName`)と、もとからそのシリーズの本は巻を残す。
シートで同じ保存に巻も入れ直していたら、入れた巻を使う。
シリーズ名を入れて選んでいない本も変わるときの確かめ(「“…”をシリーズにしますか?」)は、AppKit の `NSAlert` を窓のシートとして
出す(2026-09-22、利用者の報告: SwiftUI の `.alert(item:)` では実物で確かめが出ず、セルでの書き換えが黙って捨てられていた。
巻のある本ほど同じ単位のほかの本を巻き込むので、この確かめを通る)。
