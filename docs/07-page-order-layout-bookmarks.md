...
- **解析した本はすべて登録**(利用者の指示 2026-09-22。それまでの「ロック = 登録」を置き換えた)。「表示されているが保存されて
  ...
  - ロックした行の値は、ロックしたときにしか書きません(`MetadataWorkspace.writeRows` の `lockChanged`)。鍵を外すと、見えていた
    値のうち**ファイル名の読みと違う欄だけ**が直した欄として残ります(値は外す前と同じ。読みは確定を外して `ProposalIndex.preview`
    で読んだもの)。以前はすべての欄を直した欄にしていたので、何も変えていないのに全部の欄が青く出ました(2026-09-22、利用者の報告)。
    鍵の印は掛ける・外すともにその場で変わり、確定する値は計算の列(`tail`)で、先に並んだ直しが行に届いてから読みます(欄の書き換えの
    確定と鍵の押下が同じ操作で届いても、直した値でロックされる)。
  - 窓の外の書き手(1 冊ぶんのシート・スマートライブラリの登録・規則の変更の読み直し・保存データの読み込みなど)の知らせで本を
    合わせるのは、DB の行の形(ロック・直した欄・ルールセット)が、この窓の知っている最後の形(`MetadataWorkspace.knownStates`。
    開いたときに読んだ・この窓が書いた・外から届いた)と違う本だけ。以前は**今の入力**と比べていたので、直しが DB へ届く前
    (直しは入力を先に変え、DB へは計算の列が済んでから書く)に知らせが来ると、DB の直す前の形へ戻していた
    (2026-09-22、利用者の報告「シリーズ名を直しても、ときどきほかの本に引っ張られて元に戻る」。同じ書き手の本が多く計算が長いほど、
    またスマートライブラリを表示しているなど窓の外で書き手が動いているほど起きやすかった)。
