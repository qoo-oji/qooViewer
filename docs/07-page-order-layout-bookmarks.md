...
ファイル名の解析ルール(使うルールセットを切り替えるだけ。自動に戻すこともできる。変更した値は捨てず、読み直しは再生成で)、メタデータを削除(ロック・下書きとも。実体の有無に関係なく)、
このフォルダを除外フォルダに追加。実体の見つからない本は灰色(判定は `BookExistenceProbe`、「本ごとの保存データを削除」ウインドウと
同じ。確かめられなかった本は「無い」にしない)。保存データそのものの削除はこの窓には置かず、「本ごとの保存データを削除」ウインドウに任せる。表紙は一覧の「コレクションの表紙」の列(以前の窓と同じ `ExportCoverCell`)。

- **ロック = 登録**(利用者の決定 2026-09-21、案 A)。直した値は DB へは書かず、青い文字の下書き(`MetadataDraftStore`、
  コンテナの Application Support/qooMeta/drafts.json。窓を閉じても残る)として持ちます。下書きは**一覧に無い本の分も捨てません**
  (2026-09-22 の監査。以前は窓を開くたびに捨てていたので、スマートライブラリを OFF にした・対象フォルダのボリュームが繋がって
  いない、だけでその本の下書きが消えた)。読めない drafts.json は隣へ写しを残してから使います。左端の鍵を掛けると、その時点の値で登録し、
  鍵が掛かっている間は欄の変更・読み直しを受け付けません。鍵を外すと DB の行を消し、見えていた値は下書きに残ります。
  取り消し(⌘Z。編集メニューの「取り消す」が `MetadataEditorUndoRouter` 経由でこの窓へ流れる)は下書きの直しだけです。
- **登録済み = すべての欄が確定**。qooMeta へは確定した内容(シリーズがあれば `.series`、無ければ `.notInSeries`)として渡すので、
  ...
  欄がすべて空の本には鍵を掛けません(DB は空の行を作らないので、印だけ付いて何も登録されなかった)。錨として、未登録の本のシリーズの組にも効きます。
