...
- **環境設定「スマートライブラリ」**(2026-09-23、`SmartLibrarySettingsView`。サイドバーの先頭グループ、「ファイルブラウザ」の次):
  ...
  **表紙の形**(`AppPreferences.smartLibraryCoverShape` / `SmartLibraryCoverShape`、2026-09-23、利用者の要望)はライブラリの
  ...
  コレクションごとにすると選び直すたびにグリッドの形が変わる)、切る形では**中央を残す**(残す位置の設定は持ち込んでいない)。
  切るのは描くときの `.aspectRatio(contentMode: .fill)` で、表紙のキャッシュは切らない(形を変えても作り直さない。
  はみ出すぶんを見込んで 1.5 倍の大きさで引く)。どの形でもセルの高さは揃う(`coverShape.heightRatio`。行の位置の割り出しと
