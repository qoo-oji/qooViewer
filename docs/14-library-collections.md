...
- **対象の本**: **スマートライブラリに登録した対象フォルダの中の本だけ**(`SmartLibraryCatalog`)。最初はライブラリの本と
  ...
  追加日は Finder の「追加日」(`addedToDirectoryDateKey`。取れなければ作成日)。対象フォルダの本はメタデータの編集ウインドウの
  対象にもなる(`folderBookIDs()`)。
- **メタデータ**: 登録済みは DB の値、未登録は qooMeta の提案(全冊をまとめて読むので番号の無いシリーズも見つかる)。
...
- **まとめる**(右の見出しの束のメニュー: まとめない / 著者 / シリーズ。保存する。2026-09-22、`SmartGrouping`):
  ...
  著者名だけ(束と揃える。著者の無い本は題)。著者の束の下の 2 行目はその著者のシリーズ(1 つなら名前、
  複数なら「N シリーズ」)。以下はシリーズの場合。2 冊以上あるシリーズを 1 つの束にする
...
- **速さ**(2026-09-22。2,439 冊・Debug で実測): 前回の一覧を `Application Support/SmartLibrary/catalog.json` に保存し、
  ...
  1 冊ごとにサーバーとの往復だった(つながっていないときも保存してある表紙が出る)。qooMeta は `ProposalIndex` を持ち続け、最初は `load` で
  並列に読み(1.3 秒 → 0.4 秒)、以後は足した・消した・登録を変えた本だけ `apply` で渡す。ルールセットの自動の選択も新しい本だけ、
  並列に(0.68 秒 → 約 0.3 秒)。集め直しは 1 本ずつ順に(前のものを取り消して終わりを待つ ―― 索引と「最後に渡した本」を揃えるため)。
...
- 登録(`registerParsed`)は、この起動中に利用者が消した行を作り直さない(`BookMetadataStore.deletedThisSession`)。ルールセットを
  自動に戻した本は、前の入力を使い回さず自動の選択を選び直す(`indexedOverrides`)。起動時の刈り込みは、最後に探しきった一覧
  (catalog.json)に無い対象フォルダの中の読みだけの行も消す。
