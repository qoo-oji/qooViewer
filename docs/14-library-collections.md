...
(`WelcomeMode.smart`。ライブラリ機能が ON のときだけ。ファイルブラウザが OFF でも選べる)。StackNest のスマートシェルフが土台で、
...
- **シリーズでまとめる**(右の見出しの束のボタン。保存する。2026-09-22): 2 冊以上あるシリーズを 1 つの束にする
  (`SmartSeriesGrouping`。束の位置はその束の本が並べ替えで最初に出てきた所、束の中は巻の順。1 冊だけのシリーズは束にしない)。
  束は 1 巻目の表紙の後ろに紙を 2 枚ずらして重ね、右下に冊数バッジ。押すと束の中の本だけが並び(`openedSeries`、保存しない)、
