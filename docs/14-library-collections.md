...
ホームの帯の「スマートライブラリ」(「ファイルブラウザ」とライブラリのチップの間)で切り替える 3 つ目のモード
...
- **対象の本**: ライブラリの本 + よく使う項目のフォルダの中の本 + スマートライブラリに登録した対象フォルダの中の本
  (`SmartLibraryCatalog`。どれを含めるかは左ペインで切り替え)。フォルダの中は `SmartLibraryScanner` が FileIO の上で探す
  (書庫・PDF・EPUB と、直下に画像を持つフォルダ。TCC の保護下は起点が中にあるときだけ入る。深さと項目数に上限)。
- **メタデータ**: 登録済みは DB の値、未登録は qooMeta の提案(全冊をまとめて読むので番号の無いシリーズも見つかる)。
  読書位置(最後に読んだ日・読んだ割合)とお気に入りも集めた時点の値を持つ。
- **スマートシェルフ**(`SmartShelf`): 名前と、平らな条件の並びに「すべて / いずれか」。欄(題・著者・ジャンル・シリーズ・原作・
  イベント・情報・ファイル名・ライブラリ・コレクション・巻・読んだ割合・追加日・最後に読んだ日・種類・読書の状態・登録の有無・
  お気に入り)と演算子は `Models/SmartLibrary.swift`。保存は `SmartLibraryStore`(UserDefaults `qooViewer.smartLibrary.store`。
  対象フォルダはパスだけで、権限は FolderAccessStore。アプリ自身の移動に付いていく)。
- **重なり方**: スマートシェルフ → 絞り込み(種類・読書の状態・追加日・最後に読んだ日・登録の有無) → ブラウザ列(3 列。左の選択で
  右の候補が絞られる) → 検索。すべて AND(`SmartLibraryViewState` が作り置きする)。
- 本を集め直すのは画面が出ている間だけ(`activate` / `deactivate`)。フォルダを探し直すのは、探す場所が変わったとき・アプリ自身が
